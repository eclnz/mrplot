...
class MRISlices:
    ...
    def from_nibabel(
        ...
        if mask is not None:
            ...
            if canonical_mri.shape[:3] != canonical_mask.shape[:3]:
                raise ValueError(
                    "MRI images and mask are incompatible sizes in the first three dimensions."
                )

        if len(shape) < 3:
            raise ValueError(
                "The image must have at least 3 dimensions to extract slices."
            )
        ...
        if len(slice_locations) != 3:
            raise ValueError(
                "slice_locations must be a tuple of three integers (axial, coronal, sagittal)."
            )
        ...
        slices = [
            (
                ...
                if cropping_bounds and bounds is not None
                else slice(None)
            )
        ...
        axial_slice = (
            ...
            slices[1],
            slice_locations[2],
        )  # Axial: (x, y, specific z)
        coronal_slice = (
            slices[0],
            slice_locations[1],
            slices[2],
        )  # Coronal: (x, specific y, z)
        sagittal_slice = (
            slice_locations[0],
            slices[1],
            slices[2],
        )  # Sagittal: (specific x, y, z)

        # Access the data and extract slices
        data = np.asarray(canonical_mri.dataobj)  # Memory-mapped view

        # Extract slices
        axial = data[axial_slice].copy()  # Only copy the needed slices
        coronal = data[coronal_slice].copy()
        sagittal = data[sagittal_slice].copy()

        if mask is not None and apply_mask:
            mask_data = np.asarray(canonical_mask.dataobj)

            mask_axial = mask_data[axial_slice].copy()
            mask_coronal = mask_data[coronal_slice].copy()
            mask_sagittal = mask_data[sagittal_slice].copy()
            ...
            axial = axial * mask_axial
            coronal = coronal * mask_coronal
            sagittal = sagittal * mask_sagittal
    ...
    def apply_mask(self, mask_slices: "MRISlices") -> None:
        ...
        if self.axial.shape != mask_slices.axial.shape:
            raise ValueError(
                f"Axial slice dimensions do not match: {self.axial.shape} vs {mask_slices.axial.shape}"
            )
        if self.coronal.shape != mask_slices.coronal.shape:
            raise ValueError(
                f"Coronal slice dimensions do not match: {self.coronal.shape} vs {mask_slices.coronal.shape}"
            )
        if self.sagittal.shape != mask_slices.sagittal.shape:
            raise ValueError(
                f"Sagittal slice dimensions do not match: {self.sagittal.shape} vs {mask_slices.sagittal.shape}"
            )

        # Apply the mask to each slice
        self.axial = self.axial * mask_slices.axial
        self.coronal = self.coronal * mask_slices.coronal
        self.sagittal = self.sagittal * mask_slices.sagittal
    ...
    def add_titles_and_generate_images(
        ...
        planes = {
            "axial": (
                self.axial[..., slice_timepoint]
                if slice_timepoint is not None
                else self.axial
            ),
            "coronal": (
                self.coronal[..., slice_timepoint]
                if slice_timepoint is not None
                else self.coronal
            ),
            "sagittal": (
                self.sagittal[..., slice_timepoint]
                if slice_timepoint is not None
                else self.sagittal
            ),
        }
        ...
        for plane_name, slice_data in planes.items():
            fig_width, fig_height = self.calculate_compatible_figure_size(
                slice_data, config=config
            )

            if slice_data.ndim > 2:
                # Handle displacement images
                underlay_data = (
                    getattr(underlay_slice, plane_name) if underlay_slice else None
                )
                disp1, disp2 = extract_in_plane_displacement(
                    slice=slice_data, plane=plane_name
                )
            ...
            ax.imshow(
                slice_data,
                cmap="gray",
                vmin=self.intensity_bounds[0],
                vmax=self.intensity_bounds[1],
            )
    ...
    def calculate_compatible_figure_size(
        self, slice_array: NDArray, config: PlotConfig
    ) -> Tuple[float, float]:
        """
        ...
        scale_ratio = min(
            config.max_fig_size / fig_width_px, config.max_fig_size / fig_height_px
        )
        ...
        macro_block_size = 16
        fig_width_px = int(
            (fig_width_px + macro_block_size - 1) // macro_block_size * macro_block_size
        )
        fig_height_px = int(
            (fig_height_px + macro_block_size - 1)
            // macro_block_size
            * macro_block_size
        )
...
def plot_displacement_vectors(
    ...
    X_down = X[
        :: config.displacement_downsample_factor,
        :: config.displacement_downsample_factor,
    ]
    Y_down = Y[
        :: config.displacement_downsample_factor,
    ...
    colors[:, :, 3] = np.where(
        zero_disp_mask, 0, colors[:, :, 3]
    )  # Set alpha to 0 for zero vectors
    ...
    if underlay_image is not None:
        ...
        underlay_image = (underlay_image - underlay_image.min()) / (
            underlay_image.max() - underlay_image.min()
        )
    ...
    if underlay_image is not None:
        if underlay_image.ndim == 2:
            ...
            display_image = underlay_image[
                :, :, 0
            ]  # Reduce to 2D by taking the first slice along the third dimension
        elif underlay_image.ndim == 4:
            display_image = underlay_image[
                :, :, 0, 0
            ]  # Reduce to 2D by taking the first slice along the third and fourth dimensions
        else:
    ...
    Q = ax.quiver(
        ...
        scale_units="xy",
        scale=1,
        width=config.displacement_arrow_thickness,
    )
...
def extract_in_plane_displacement(
    slice: NDArray, plane: str
    ...
    if plane == "sagittal":
        ...
        raise ValueError(
            f"Invalid plane '{plane}'. Must be one of: 'sagittal', 'coronal', 'axial'."
        )
    return disp1, disp2


def extract_image_from_canvas(
    fig: plt.Figure, discard_alpha: bool = True
) -> np.ndarray:
    ...
    img_array: NDArray[np.uint8] = buffer.reshape(
        (height, width, 4)
